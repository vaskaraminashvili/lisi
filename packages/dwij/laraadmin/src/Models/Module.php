...
class Module extends Model
{
	...
	public static function create_field_schema($table, $field, $update = false, $isFieldTypeChange = false) {
		...
		switch ($field->field_type) {
			...
			case 'Checkbox':
				if($update) {
					$var = $table->boolean($field->colname)->nullable()->change();
				} else {
					$var = $table->boolean($field->colname)->nullable();
				}
			...
			case 'Currency':
				if($update) {
					$var = $table->double($field->colname, 15, 2)->nullable()->change();
				} else {
					$var = $table->double($field->colname, 15, 2)->nullable();
				}
				if($field->defaultvalue != "") {
					$var->default($field->defaultvalue);
				} else if($field->required) {
			...
			case 'Datetime':
				if($update) {
					...
					$var = $table->timestamp($field->colname)->nullable();
				}
			...
			case 'Decimal':
				$var = null;
				if($update) {
					$var = $table->decimal($field->colname, 15, 3)->nullable()->change();
				} else {
					$var = $table->decimal($field->colname, 15, 3)->nullable();
				}
				if($field->defaultvalue != "") {
					$var->default($field->defaultvalue);
				} else if($field->required) {
			...
			case 'File':
				if($update) {
					$var = $table->integer($field->colname)->nullable()->change();
				} else {
					$var = $table->integer($field->colname)->nullable();
				}
			...
			case 'Files':
				if($update) {
					$var = $table->string($field->colname, 256)->nullable()->change();
				} else {
					$var = $table->string($field->colname, 256)->nullable();
				}
				if(is_string($field->defaultvalue) && starts_with($field->defaultvalue, "[")) {
					$var->default($field->defaultvalue);
			...
			case 'Integer':
				$var = null;
				if($update) {
					$var = $table->integer($field->colname, false)->unsigned()->nullable()->change();
				} else {
					$var = $table->integer($field->colname, false)->nullable()->unsigned();
				}
				if($field->defaultvalue != "") {
					$var->default($field->defaultvalue);
				} else if($field->required) {
